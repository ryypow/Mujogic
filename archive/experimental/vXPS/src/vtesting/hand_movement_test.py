...
def main():
    ...
    #target joint positions for hand actuator
    ...
    with mujoco.viewer.launch_passive(sim.model, sim.data) as viewer:
        ...
        while viewer.is_running():
            ...
            time.sleep(2.0)
''''''
    def print_hand_map(sim):
        # blocks by construction
        finger_blocks = {
            "index (finger1)":  ["1","0","2","3"],
            "middle (finger2)": ["5","4","6","7"],
            "ring (finger3)":   ["9","8","10","11"],
            "thumb":            ["12","13","14","15"],
        }
        tip_sites = {
            "index (finger1)": "fingertip1_site",
            "middle (finger2)": "fingertip2_site",
            "ring (finger3)": "fingertip3_site",
            "thumb": "thumb_fingertip",
        }
        for fname, jnames in finger_blocks.items():
            print(f"\n{fname}:")
            for jn in jnames:
                jid = mujoco.mj_name2id(sim.model, mujoco.mjtObj.mjOBJ_JOINT, jn)
                lo, hi = sim.model.jnt_range[jid]
                print(f"  joint {jn}: range {lo:.3f} → {hi:.3f} rad")
            sid = mujoco.mj_name2id(sim.model, mujoco.mjtObj.mjOBJ_SITE, tip_sites[fname])
            pos = sim.data.site_xpos[sid]
            print(f"  tip site {tip_sites[fname]} @ {pos}")
            tip_sites = ["fingertip1_site","fingertip2_site","fingertip3_site","thumb_fingertip"]
            for sn in tip_sites:
                sid = mujoco.mj_name2id(sim.model, mujoco.mjtObj.mjOBJ_SITE, sn)
                print(sn, sim.data.site_xpos[sid])

    def print_actuator_bindings(sim, joint_names):
        for jn in joint_names:
            jid = mujoco.mj_name2id(sim.model, mujoco.mjtObj.mjOBJ_JOINT, jn)
            an = None
            for a in range(sim.model.nu):
                if sim.model.actuator_trnid[a,0] == jid:
                    an = mujoco.mj_id2name(sim.model, mujoco.mjtObj.mjOBJ_ACTUATOR, a)
                    break
            print(f"joint {jn} -> actuator {an}")
