...
class Simulation:
    ...
    def load(self): #loads XML scene
        ...
        self.data.qpos[qposadr] = -math.pi/2 #initial position of shouldeR_lift_joint
        mujoco.mj_forward(self.model, self.data)
    
    def ids_by_name(self, names, kind, role): #populates arm_joint_names/ID's
        out = []
        for n in names:
    ...
    def step(self, viewer, steps=100): #advances the sim and viewer.sync() each tick
...
######
    ...
    def init_renderer(self, width=640, height=480): #uses MuJoCo 3.x with given width/height
    ...
    def cam_id(self, name: str) -> int: #gets camera ID from a named camera in the XML scene
        self.cid = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_CAMERA, name)
        if self.cid < 0:
            raise ValueError(f"Camera '{name}' not found.")

    def _compute_intrinsics(self, cam_id_int: int, width: int, height: int): #pinhole intrinsics
    ...
    def capture_rgb(self, width=640, height=480) -> np.ndarray: #render RGB
        mujoco.mj_forward(self.model, self.data)
    ...
    def capture_depth(self, width=640, height=480) -> np.ndarray: #linear depth
        mujoco.mj_forward(self.model, self.data)
        self.init_renderer(width, height)
    ...
    def capture_labels(self, width=640, height=480) -> np.ndarray: #Segmentaton ID's
        mujoco.mj_forward(self.model, self.data)
        self.init_renderer(width, height)
...
#One-stop call: get_frame computes intrinsics, returns a dictionary with rgb, depth, depth_vis, seg_vis
    ...
    def save_frame(self, frame: dict, prefix="frame", outdir=None): #writes the PNGs with OpenCV (if available) -- uses NPZ as fallback
        import os
    ...
    def set_gripper_angles(self, q_target): #write control inputs
        ...
        for i, act_id in enumerate(self.hand_act_ids):
            self.data.ctrl[act_id] = q_target[i] #controls for each hand actuator


    def move_gripper_to_angles(self, q_target, duration=1.0, viewer=None): 
        #linearly interpolates the target angles over duration 
        #and steps the sim--makes the gripper move smoothly
    ...
    #computes arm joint angles that place a site at target_position with target_euler_xys oritentation
    def desired_qpos_from_ik(self, site_id,
                            target_pos, target_euler_xyz):
        physics = dm_mujoco.Physics.from_xml_path(self.scene_path) #builds mujoco.physics object from scene xml
    ...
    def actuators_for_joints(self, role): #looks up actuators that drive each joint and caches them for arm/hand
        act_ids = []
        ...
        for j in joint_ids:
            ...
            for a in range(self.model.nu):
                if self.model.actuator_trnid[a,0] == j:
                    found = a
                    break
            if found is None: #if a joint has no actuator, raises error--makes sure XML is wired
    ...
    def arm_qpos(self): #snapshots current arm vector
    ...
    def _cubic_blend(self, s): #cubic bend is a standard smoothstep for jerk-reduced interpolation
        s = max(0.0, min(1.0, s))
        return 3*s*s - 2*s*s*s
    
    def _set_ctrl_arm_qdes(self, qdes): #writes desired joint values to arm actuator controls each step
        for a, v in zip(self.arm_act_ids, qdes):
            self.data.ctrl[a] = float(v)

    #creates a waypoint list from current q to goal q. shortest_Wrap wraps angles into (-pi, pi) to avoid long rotation on circular joints
    def plan_straight_joint_path(self, q_goal, n_waypoints=80, shortest_wrap=True): #creates a waypoint list from current q to goal q
        q0 = self.arm_qpos()
    ...
    def execute_straight_joint_move(self, q_goal, duration=2.0, n_waypoints=80, viewer=None): #plan + execute with total duration budgeting
