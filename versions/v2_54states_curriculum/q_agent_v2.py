...
def state_translator(observation, env):
    ...
    cumulative_rot = base_env.cumulative_rotation
    goal_abs = abs(base_env.rotation_goal_delta)
    goal_progress = abs(goal_abs - cumulative_rot)
    ...
    return state_id, progress_bin, grasp_bin, speed_bin, cumulative_rot, goal_progress
...
base_env = CanRotateEnv(target_degrees=initial_goal, render_mode="human")
...
for episode in range(NUM_EPISODES):
    ...
    if episode > 0 and episode % episodes_per_goal == 0:
        EPSILON = max(0.3, EPSILON)  # Boost exploration for new goal
        print(f"  -> Curriculum advanced! Epsilon reset to {EPSILON:.2f}")
    ...
    while step < MAX_STEPS:
        ...
        if terminated or truncated:
            best_future_q = 0  # No future after episode ends
        else:
    ...
    success = goal_prog < np.deg2rad(5)
    ...
    if (episode + 1) % 50 == 0:
        ...
        success_rate = np.mean(success_tracker[-50:]) * 100
        print(f"Ep {episode+1:4d}/{NUM_EPISODES} | Goal: {current_goal:3d}° | "
            f"Avg Reward: {avg_reward:7.2f} | Success: {success_rate:5.1f}% | "
